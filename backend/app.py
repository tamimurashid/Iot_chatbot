...
def check_and_alert():
    ...
    for user in users:
        ...
        for event in events:
            ...
            message = event.get("message", f"⚠️ Alert! Condition met for {param}")
            ...
            if evaluate_condition(sensor_value, condition):
                full_message = f"⚠️ {param.upper()} = {sensor_value}, condition: {condition}\n{message}"
...
def update_device_data():
    ...
    if result.modified_count == 0:
     ...
     # ✅ Call alert check after updating
    alerts = check_and_alert()

    return jsonify({"status": "success", "message": "Data updated", "alerts": alerts})
